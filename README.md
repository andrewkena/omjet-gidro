...
Настольное окно (PySide6, иконка `assets/gidro.ico`, версия — в заголовке окна и
в константе `APP_VERSION`) для быстрого просмотра трека эхолота на карте, построения
...
- **«Линии изобат»** — толщина и цвет линий (выбор через `QColorDialog`).
- **«Подписи изобат»** — размер шрифта подписей и частота (через сколько точек
  линии повторять подпись глубины); тем же шрифтом подписываются и диапазоны
  глубин внутри залитых областей (см. ниже), по одной подписи в центре области.
- **«Заливка изобат»** — прозрачность заливки и число ступеней (диапазонов глубин,
  каждый залит своим цветом по градиенту).
...
- **«Удалить изобаты»** — убирает с карты построенные линии, заливку и подписи
  (трек и урез воды остаются).
