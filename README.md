...
Настольное окно (PySide6, иконка `assets/gidro.ico`, размер по умолчанию 1100×1000,
...
позиций. Если загружен ровно один файл эхограммы, заодно уточняются «Начало» и
«Конец» трека в сводке — настоящим UTC-временем из синхронизации с GNSS (см. ниже,
почему это надёжнее времени файла).
...
- Клик по карте в окне изобат (рисование уреза воды) передаётся из JS в Python через
  `QWebChannel` (`MapClickBridge`) — единственное место в проекте, где нужна связь
  JS → Python; во всех остальных картах данные идут только Python → JS (перегенерацией
  HTML или `runJavaScript`).
