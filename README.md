...
- **Посмотреть эхограмму** — отдельное окно с водопадом сонара по выбранному файлу
  (столбец — пинг, строка — амплитуда по глубине, чёрно-белое изображение с
  прокруткой). Слева — линейка глубин, снизу — ось времени/расстояния (берётся
  из настройки «Временная шкала»). Колесо мыши над изображением — масштаб
  (0.1×–20×), ползунок «Контраст» — усиление контраста амплитуды. Формат этих
  байт нигде не задокументирован и не проверен на реальном железе — реализовано
  по общепринятому для .sl2 предположению (см.
  `read_echogram_waterfall` в gui.py и docs/CONTEXT.md).
...
### Карта и таймлайн
- Точки раскрашены по глубине радужной палитрой (минимум — красный, максимум —
  синий); чекбокс «Глубина» отключает раскраску (тогда все точки одного цвета).
  Легенда — вертикальная, тёмная полупрозрачная, в правом нижнем углу карты, с
  подписанными границами диапазона.
- Наведение курсора на точку показывает её глубину во всплывающей подсказке и
  подсвечивает соответствующее место на таймлайне под картой.
